...
class Material:
    ...
    def initSilica(self):
        self.A = [0.696166, 0.407943, 0.897479]
...
class Mode:
    ...
    def __init__(self, diameter, wavelength, matCore="silica", matCladd="air", nu=1):        
        self.matCore = Material(mat="silica")
        self.matCladd = Material()
    ...
    def plotMode(self,xmin=-5,xmax=5, ymin=-5, ymax=5, npts=256):
        ...
        x = plt.imshow(Z, extent=[xmin,xmax,ymin,ymax], cmap='jet', origin='lower', interpolation='none')
        plt.colorbar()
        plt.show()
...
class TMMode(Mode):
    ...
    def eigenValuePb(self, U):
        ...
        nco2 = self.nCore**2
        ncl2 = self.nCladd**2
        return nco2*jv(1,U)/(U*jv(0,U)) + ncl2*kv(1,w)/(w*kv(0,w))
...
class HybridMode(Mode):
    ...
    def HE_mode(self, U):
        ...
        return jv(self.nu-1,U)/(U*jv(self.nu,U)) + KK - self.nu/(U*U) + np.sqrt(R2)
